"""Twenty questions about the released dataset, and what is wrong with it.

Split out of release_numbers.py, which was two unrelated programs in 607 lines: a table of
macros the manuscript quotes, and this -- a data-quality battery over the same file. They shared
nothing but the loader, and neither was in checks/run.py.
...
Nothing here is filtered out of the release. A record that fails a check stays in the data with
the reason recorded, so the filter can be argued with instead of taken on trust.

    quality.py              every check
    quality.py --show 20    more of the offending rows
"""
import argparse
import re
import sys
...
from core.schema import OTHER_ROUTE, ROUTE_FROM_SOLVENT
from release_numbers import RELEASE, load
...
def inference(d):
    ...
    report(16, f"rows routed '{OTHER_ROUTE}' only because of the abbreviation used",
           int(blind.sum()), int((d.route == OTHER_ROUTE).sum()),
    ...
    def fired(value):
        ...
        return [label for label, pattern in ROUTE_FROM_SOLVENT if re.search(pattern, text)]
