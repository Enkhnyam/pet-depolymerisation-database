"""How much more adjudication would it take to name the better grader?

Only records where exactly one grader matched the chemists carry information; where both agree,
the record cannot separate them. That makes the design cheap: a record the two graders already
disagree on is *guaranteed* to be informative once a human labels it, because one of them must
be the one that matched. So the labelling budget should be spent on disagreements, not on a
random sample.

Power here is for McNemar's exact test, which on discordant pairs is a two-sided binomial test
against p = 0.5.
"""
import pandas as pd
...
def main() -> None:
    ...
    print(f"\n  p = {binomtest(int(judge_only), have).pvalue:.3f}; "
          f"with {have} pairs the smallest p reachable is "
          f"{binomtest(have, have).pvalue:.3f}, so no outcome here could reach {ALPHA}")
    ...
    show(f"the pool, {judge_name} judging {target}", {
        "records": len(both),
        "the graders disagree on": len(disagree),
        "  of those, the metric could evaluate": len(ev_disagree),
    }, fmt="{:.0f}")
    print("\n  every disagreement is informative once labelled, so the pool is the budget")
