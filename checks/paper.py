...
The manuscripts are not in this repository -- the journal publishes those -- so this check has
two modes and picks one by looking. With no .tex present, as on a clone, it verifies the part
that is always here: that every macro artifacts/paper_numbers.tex defines traces back to a check
and a run bundle, so nothing can be quoted that nothing computed. With the manuscripts beside it,
as on the author's machine, it also reads their body text, which is the mode the rest of this
docstring describes.
...
scripts/derive.sh and both already refuse to go stale. The hole between them is the body text: a
...
REVIEWED = {
    ...
    ("GaoExtraFields", "10"): "the heuristic grader's ten evaluated fields, a different count "
                              "in a different section that happens to land on the same ten",
    ("CorrSampled", "120"): "the 120 in gpt-oss-120b, which is a model name and not a count",
...
def body(text: str) -> str:
    ...
    # Verbatim blocks are quoted source, not prose. The SI now prints the extraction and judge
    # prompts in full, and those carry fabricated worked examples -- "Ref. 21", "conversion 92",
    # "20 g EG" -- that collided with \CorrRenameBoth, \EmptyBiological and
    # \ThresholdTolerancePercent. A number inside a prompt is the prompt's, and replacing it
    # with a macro would change the text the model was actually given.
    text = re.sub(r"\\begin\{(Verbatim|verbatim|lstlisting)\}.*?\\end\{\1\}", "",
                  text, flags=re.S)
