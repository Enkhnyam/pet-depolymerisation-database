...
"""
from _style import (DATA, EMPHASIS, ROUTE, SLOTS, canvas, headroom, pie, ranked_bars, save,
                    step_hist)
...
def main() -> None:
    ...
    pie(panel[1], ordered, colours=[ROUTE[name] for name in ordered.index])
    panel[1].set_xlabel(f"{int(ordered.sum()):,} records by route")
    ...
    panel[5].set_visible(False)          # five panels in a six-cell grid
